...
public class MyPageServiceImpl implements MyPageService{
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws IOException{
		...
		*/
		...
		// 위 두 경로를 연결해주는 코드를 FileConfig.java에 registry 라고 쓰였기때문에 가능함
...
//		uf.setFileOriginName(uploadFile.getOriginalFilename());
//		uf.setFileRename(fileRename);
		...
		// C:/uploadFiles/test/변경한파일명 으로
		// 파일을 서버 컴퓨터에 저장
		
		uploadFile.transferTo(new File(folderPath + fileRename));
							// C:/uploadFiles/test/200417111603_00001.png
		// -> CheckedException 발생 -> 예외 처리 필수
		
		// @Transactional은 RuntimeException(UncheckedException)만 처리
		// -> rollbackFor 속성 이용해서
		// 롤백할 예외 범위를 수정
	...
	public int fileUpload3(List<MultipartFile> aaaList,
		...
		for(MultipartFile file : aaaList) {
			...
			// -> 파일 하나 업로드 + DB INSERT
			result1 += fileUpload2(file, memberNo); // 1행이 들어간다
		}
		...
		for(MultipartFile file : bbbList) {
			...
			// fileUpload2() 메서드 호출(재활용)
			// -> 파일 하나 업로드 + DB INSERT
			result2 += fileUpload2( file, memberNo); // 1행이 들어간다
		}
