...
public class EmailServiceImpl implements EmailService {
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			// 1번 매개변수 : MimeMessage
			// 2번 매개변수 : 파일 전송 사용 할거냐 말거냐 true / false
			// 3번 매개변수 : 문자 인코딩 지정
			...
			// HTML 코드 해석 여부 true (innerHTML 해석)안하면 <br> 적용 안됨
			
			// CID(Content-ID)를 이용해 메일에 이미지 첨부
			// (파일첨부와는 다름, 이메일 내용 자체에 사용할 이미지 첨부)
			// logo 추가예정
			// ClassPath 에 있는 자원중 static/images/logo.jpg를 log라는 이름으로 cid에 등록한다
		...
		// 이메일 하나당 인증번호 하나를 보내야 받는사람이 인증번호 찾기 변함
		// 수정(update)을 먼저 실행
	...
	// setText는 string형임 html을 보낼때 html을 string으로 형변환 해야함
	private String loadHtml(String authKey, String htmlName) {
		
		// org.thymeleaf.Context 선택
		Context context = new Context();
		// 04/12 09:22
		// thymledaf 가 적용된 html을 사용할수 있는 객체(signup.html)
		
		// 타임리프가 적용된 HTML에서 사용할 값 추가
		context.setVariable("authKey", authKey);
		
		
		// html을 자바로 변환하는 객체를 만들어야하는데 그걸 spring에서 관리함(templateEngine)
		// templates/email 폴더에서 htmlName과 같은 
		// ~.html 파일 내용을 읽어와서 String으로 변환
...
--> 이메일 메시지를 보내고 받을 때 사용하는 약송(규약, 방법)
...
build.gradle 에
implementation 'org.springframework.boot:spring-boot-starter-mail' 라고 있다
