...
@Slf4j
@Component // Bean 등록, 스케쥴을 쓰려면 클래스를 자바가 관리할수 있게 객체화(인스턴스화)해야함
...
public class ImageDeleteScheduling {
	...
	// @Scheduled(cron = "0 0 * * * *") // 정시 마다 수행, 1시 2시 3시...
	...
	public void scheduling() {
		...
		// imageArr 라는 빈 배열을 boardArr와 memberArr 깊이 만큼의 크기로 만든다
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복사 (깊은 복사)
		// System.arraycopy(복사할배열, 몇번인덱스부터복사할지, 새로운배열, 새로운 배열의몇번부터넣을지인덱스, 복사를어디까지할건지)
