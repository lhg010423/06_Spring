...
memberEmail.addEventListener("input", e => {
    ...
    if(inputEmail.trim().length === 0) {
        ...
        // signup.html 에서 id="emailMessage" 의 값이랑 같아야함
...
// setInterval : 일정 시간마다 안에 함수를 실행해줌
...
sendAuthKeyBtn.addEventListener("click", () => {
    ...
    authTimer = setInterval( () => {
        ...
        authKeyMessage.innerText = `${addZero(min)}:${addZero(sec)}`; // addZero함수생성
...
memberPw.addEventListener("input", e => {
    ...
    if(!regExp.test(inputPw)) { // 유효하지 않으면 실행
...
memberNickname.addEventListener("input", (e) => {
    ...
    // 3) 중복 검사 (유효한 경우)
    nickMessage.innerText = "유효한 이메일 형식입니다";
